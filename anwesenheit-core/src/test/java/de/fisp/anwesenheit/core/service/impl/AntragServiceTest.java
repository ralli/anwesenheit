...
public class AntragServiceTest {
    ...
    public void antragAnlageOhneBerechtigung() {
        ...
        when(berechtigungsService.isAntragEigentuemerOderErfasser(isA(Antrag.class), eq(benutzerId))).thenReturn(false);
        try {
            antragService.createAntrag(benutzerId, cmd);
        } catch (NotAuthorizedException ex) {
            return;
    ...
    public void antragAnlageErfolgreich() {
        ...
        cmd.setBenutzerId(benutzerId);
        cmd.setBewilliger(new String[]{"chef", "boss"});
        when(berechtigungsService.isAntragEigentuemerOderErfasser(isA(Antrag.class), eq(benutzerId))).thenReturn(true);
        antragService.createAntrag(benutzerId, cmd);
    }
}
